...
def partition(source: ObservableBase, predicate: Predicate = None,
    ...
    return [
        published.filter(predicate=predicate, predicate_indexed=predicate_indexed),
        published.filter(predicate_indexed=lambda x, i: not (predicate(x) if predicate else predicate_indexed(x, i)))
    ]
