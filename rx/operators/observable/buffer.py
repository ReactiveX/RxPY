...
def buffer(source, buffer_openings=None, buffer_closing_selector=None) -> ObservableBase:
    ...
    return source.window(buffer_openings, buffer_closing_selector).flat_map(lambda item: item.to_iterable())
...
def buffer_with_count(source, count, skip=None) -> ObservableBase:
    ...
    return source.window_with_count(count, skip).flat_map(selector).filter(predicate)
