...
def if_then(condition: Callable[[], bool], then_source: Observable,
    ...
    else_source = else_source or StaticObservable.empty()

    then_source = StaticObservable.from_future(then_source) if is_future(then_source) else then_source
    else_source = StaticObservable.from_future(else_source) if is_future(else_source) else else_source
