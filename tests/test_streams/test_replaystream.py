...
def test_infinite():
    ...
    def action1(scheduler, state=None):
        stream[0] = ReplayStream(3, 100, scheduler)
    scheduler.schedule_absolute(100, action1)

    def action2(scheduler, state=None):
        subscription[0] = xs.subscribe(stream[0])
...
def test_stream_disposed():
    ...
    def action1(scheduler, state=None):
        stream[0] = ReplayStream(scheduler=scheduler)
    scheduler.schedule_absolute(100, action1)
...
def test_replay_stream_dies_out():
    scheduler = TestScheduler()
    ...
    def action1(scheduler, state=None):
        stream[0] = ReplayStream(sys.maxsize, 100, scheduler)
    scheduler.schedule_absolute(100, action1)

    def action2(scheduler, state=None):
        xs.subscribe(stream[0])
    scheduler.schedule_absolute(200, action2)

    def action3(scheduler, state=None):
        stream[0].subscribe(results1)
    scheduler.schedule_absolute(300, action3)

    def action4(scheduler, state=None):
        stream[0].subscribe(results2)
    scheduler.schedule_absolute(400, action4)

    def action5(scheduler, state=None):
        stream[0].subscribe(results3)
    scheduler.schedule_absolute(600, action5)

    def action6(scheduler, state=None):
        stream[0].subscribe(results4)
    scheduler.schedule_absolute(900, action6)
