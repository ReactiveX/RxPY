...
from reactivex import operators as ops, interval
...
class TestSwitchMap(unittest.TestCase):
    def test_switch_map_uses_index(self):
        ...
        def create():
            return xs.pipe(ops.switch_map(
                project=create_inner
            ))
    ...
    def test_switch_map_inner_throws(self):
        ...
        sources = [
            ...
            scheduler.create_cold_observable(on_next(50, 'wont happen'), on_error(120, "no")),
        ]
        xs = scheduler.create_hot_observable(
            on_next(
                250,
        ...
        assert sources[1].subscriptions == [
    ...
    def test_switch_map_no_inner(self):
        ...
        sources = [
           scheduler.create_cold_observable(on_next(20, 2))
        ]
    ...
    def test_switch_map_inner_completes(self):
        ...
        assert results.messages == [
            on_next(320, (0,0,'d')),
            on_next(350, (1,0,'f')),
            on_next(370, (1,1,'f')),  # here the current inner is unsubscribed but not the outer
