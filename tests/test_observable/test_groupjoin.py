...
class TestGroup_join(unittest.TestCase):
    ...
    def test_group_join_op_error_iii(self):
        ...
        def create():
            return xs.group_join(ys,
                lambda x: Observable.timer(x.interval).select_many(Observable.throw_exception(ex) if x.value==6 else Observable.empty()),
                lambda y: Observable.timer(y.interval),
                lambda x, yy: yy.map(lambda y: str(x.value) + y.value)
            ).merge_observable()
        results = scheduler.start(create=create)
    ...
    def test_group_join_op_error_iv(self):
        ...
        def create():
            return xs.group_join(ys,
                lambda x: Observable.timer(x.interval),
                lambda y: Observable.timer(y.interval).select_many(Observable.throw_exception(ex) if y.value=="tin" else Observable.empty()),
                lambda x, yy: yy.map(lambda y: str(x.value) + y.value)
            ).merge_observable()
        results = scheduler.start(create=create)
