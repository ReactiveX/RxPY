...
class TestTakeLast(unittest.TestCase):
    ...
    def test_takelast_some3(self):
        ...
        xs = scheduler.create_hot_observable(
                ...
                on_next(270, 7), on_next(280, 8), on_next(290, 9),
                on_completed(300))
        ...
        assert res.messages == [
                on_next(300, 6), on_next(300, 7), on_next(300, 8),
                on_next(300, 9), on_completed(300)]
        assert xs.subscriptions == [subscribe(200, 300)]

    def test_takelast_some4(self):
        scheduler = TestScheduler()
        xs = scheduler.create_hot_observable(
                on_next(210, 1), on_next(240, 2), on_next(250, 3),
                on_next(280, 4), on_next(290, 5), on_next(300, 6),
                on_completed(350))
